...
public class LtColumnServiceImpl implements LtColumnService{
	@Autowired
	...
	public String findKeywordsByNetwork(Integer networkid){
		return columnDAO.findKeywordsByNetwork(networkid);
	}
	//根据用户ID以及类型搜索微信关键词设置
	public List<LtColumn> listByNetwork(Integer networkid, int type){
		return columnDAO.listByNetwork(networkid, type);
	}
	...
	public HashMap<Integer, LtColumn>hashByNetwork(Integer networkid, Integer type){
		List<LtColumn> list = columnDAO.listByNetwork(networkid, type);
		HashMap<Integer, LtColumn> hash = new HashMap<Integer, LtColumn>();
		for(int i=0; i<list.size(); i++){
			LtColumn column = list.get(i);
	...
	public void save(HttpServletRequest request, Network network, String prefix){
		...
		List<LtColumn> list = columnDAO.listByNetwork(network.getId(), 0);
		
		for(int i=0; i<6; i++){
			LtColumn column = new LtColumn();
			if(list.size()>i){
				column = list.get(i);
			}
	...
	public void refresh(String prefix, LtColumn column){
		...
		if(column!=null&&column.getSearchword()!=null&&column.getSearchword().trim().length()>0){
			...
			//更新栏目的统计数据
			Integer contenttotal = resultDAO.contentCountByColumn(column.getId());
			Integer contenttoday = resultDAO.contentCountByColumnToday(column.getId());
